...
import { ArrowLeft, MessageCircle, Loader2, AlertCircle, RefreshCw, X, ChevronDown, ChevronUp } from "lucide-react";
...
export default function EditorOpenRouterPage() {
  ...
  const [responseDetails, setResponseDetails] = useState<OpenRouterResponse['data'] | null>(null);
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      console.log('Sending request to /api/openrouter-edit...');
      
      const response = await fetch('/api/openrouter-edit', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok || !result.success) {
        ...
        if (result.error && typeof result.error === 'object') {
          ...
          setError({
            ...
            message: result.error?.message || result.error || 'Failed to process image with AI',
            details: result.error?.details || result.details || 'An unexpected error occurred',
      ...
      if (result.success) {
        ...
        console.log('AI responded successfully:', result.data.content ? result.data.content.substring(0, 100) + '...' : 'No text, checking for images...');
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-white/70">
              Upload an image and chat with AI about it - completely free
            </p>
            ...
            {currentImage && (
              <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20 space-y-6">
                <div>
                  ...
                  <Textarea
                    id="prompt"
                    placeholder="Ask questions about the image... (e.g., 'What do you see in this image?', 'Describe the colors and atmosphere', 'What's happening in this scene?', 'Analyze the composition')"
                    value={prompt}
                ...
                <Button 
                  ...
                  className="w-full py-6 text-lg font-semibold bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 disabled:from-gray-500 disabled:to-gray-600 text-white border-0"
                >
                  {isProcessing ? (
                    <>
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {error.type === 'authentication' && (
                            <div className="mt-3 p-3 bg-yellow-500/10 rounded text-sm text-yellow-200">
                              💡 Check your OpenRouter API key in the .env.local file
                            </div>
                          )}
                          ...
                          {error.type === 'content_filter' && (
                            <div className="mt-3 p-3 bg-purple-500/10 rounded text-sm text-purple-200">
                              🚫 Try modifying your prompt or using a different image that complies with content policies
                            </div>
                          )}
                          {error.type === 'length_limit' && (
                            <div className="mt-3 p-3 bg-indigo-500/10 rounded text-sm text-indigo-200">
                              📏 Try using a shorter prompt or requesting a more concise response
                            </div>
                          )}
                          {error.type === 'model' && (
                            <div className="mt-3 p-3 bg-pink-500/10 rounded text-sm text-pink-200">
                              🤖 The AI model encountered an issue. Try rephrasing your request
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  {responseDetails?.hasImages && responseDetails.images && (
                    <div className="space-y-4">
                      <h4 className="text-xl font-semibold text-white">Generated Images</h4>
                  ...
                  {responseDetails && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="bg-white/5 rounded-lg p-4">
                        <h4 className="text-white/80 font-medium mb-2">Model Information</h4>
                        <div className="space-y-1 text-white/60">
                          <p>Model: {responseDetails.model}</p>
                          <p>Response ID: {responseDetails.id}</p>
                          <p>Finish Reason: {responseDetails.finishReason}</p>
                        </div>
                      </div>
