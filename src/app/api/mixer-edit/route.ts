...
async function handleFalRequest(
  ...
  try {
    ...
    if (error && typeof error === "object") {
      ...
      if (falError.body) {
        console.error(
          "FAL AI error body details:",
          JSON.stringify(falError.body, null, 2)
        )
      }
    }
    ...
    let errorDetails =
      error instanceof Error ? error.message : "Unknown FAL AI error"
...
async function handleOpenRouterRequest(
  ...
  try {
    ...
    if (responseImages && Array.isArray(responseImages)) {
      console.log(
        "Processing images from OpenRouter response:",
    ...
    if (error && typeof error === "object" && "response" in error) {
      ...
      if (openaiError.response?.data?.error) {
        errorResponse.message =
          openaiError.response.data.error.message || errorResponse.message
        errorResponse.details =
          openaiError.response.data.error.message || errorResponse.details
        errorResponse.code =
          openaiError.response.data.error.code || errorResponse.code
      }
    }
      ...
      return NextResponse.json(
        {
          ...
          error: {
            ...
            details:
              "Please provide a description of what you want to do with the images",
            code: "NO_PROMPT",
            retryable: false,
            httpStatus: 400,
          },
